...
def clear_discovery_json(path: str = "discovery.json"):
    """Clear old discovery output before a new run."""
    try:
        if os.path.exists(path):
            os.remove(path)
    except Exception as e:
        print(f"Warning: Could not remove old {path}: {e}")


def main():
    """Main function to discover and display devices."""

    clear_discovery_json()
    ...
    try:
        ...
        print(f"Devices with services: {sum(1 for d in devices if d.services)}")
        print(f"Scan duration: {scan_duration:.2f} seconds")
